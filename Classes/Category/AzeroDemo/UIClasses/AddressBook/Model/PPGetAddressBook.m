...
{
    [kPPAddressBookHandle requestAuthorizationWithSuccessBlock:^(bool authorization) {
        if (authorization) {
...
+ (void)getOriginalAddressBook:(AddressBookArrayBlock)addressBookArray authorizationFailure:(AuthorizationFailure)failure authorizationWhetherOrNotToStop:(AuthorizationWhetherOrNotToStop)whetherOrNotToStop
{
    // 将耗时操作放到子线程
...
//                failure ? failure() : nil;
        ...
        } authorizationWhetherOrNotToStop:^(BOOL stop) {
            dispatch_async(dispatch_get_main_queue(), ^{
//                whetherOrNotToStop ?whetherOrNotToStop(stop):nil;
            });
        }];
//        [kPPAddressBookHandle getAddressBookDataSource:^(PPPersonModel *model) {
//
...
+ (void)getOrderAddressBook:(AddressBookDictBlock)addressBookInfo authorizationFailure:(AuthorizationFailure)failure authorizationWhetherOrNotToStop:(AuthorizationWhetherOrNotToStop)whetherOrNotToStop
{

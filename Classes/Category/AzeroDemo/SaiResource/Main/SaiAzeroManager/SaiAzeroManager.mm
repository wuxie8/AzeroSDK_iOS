...
@property (nonatomic ,copy) azeroManagerTtsPlayComplete azeroManagerTtsPlayompleteHandle;
@property (nonatomic ,copy) azeroManagerRecordCallBackBlock azeroManagerRecordCallBackHandle;
...
//    [self copyOpenDenoiseFileLogTo:txtDir];
    ...
    //设备id
    [self updateDictionaryItem:mutableDic andContainer:@"deviceInfo" andPath:@"deviceSerialNumber" andValue:@"123456789"];
    ...
    [[AudioQueuePlay sharedAudioQueuePlay] azeroAudioQueuePlayTtsComplete:^{
        if (self.azeroManagerTtsPlayompleteHandle) {
            self.azeroManagerTtsPlayompleteHandle();
        }
    }];
    
    [[XBEchoCancellation shared] echoCancellation_RecordCallBackBlock:^(NSDictionary *dict) {
        if (self.azeroManagerRecordCallBackHandle) {
            self.azeroManagerRecordCallBackHandle(dict);
        }
    }];
...
- (void) azeroMainLoop:(NSString *)configFile {  
    ...
    self.engine = engine;
    bool bRet = [self.engine configure:[NSArray arrayWithObjects:configFile, nil]];
    assert(bRet);
...
- (void)saiTtsPlayComplete:(azeroManagerTtsPlayComplete )ttsPlayComplete{
    if (ttsPlayComplete) {
        self.azeroManagerTtsPlayompleteHandle = ttsPlayComplete;
    }
}
- (void)saiManagerRecordCallBack:(azeroManagerRecordCallBackBlock)recordCallBackBlock{
    if (recordCallBackBlock) {
        self.azeroManagerRecordCallBackHandle = recordCallBackBlock;
    }
}
- (void)saiManagerResetAudioQueuePlay{
